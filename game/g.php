...
function generateNewRoom() {
    // Update the matrix size and symbols
    ...
    // Store the flag position in the session
    $_SESSION['flag_row'] = $flag_row;
    $_SESSION['flag_col'] = $flag_col;

    // Populate the matrix with random symbols
...
<body>
    <h1>Room Search Game - Room <?php echo $_SESSION['room_number']; ?></h1>
    <table>
        ...
        function handleKeyPress(event) {
            ...
            if (currentRow === <?php echo $_SESSION['flag_row']; ?> && currentCol === <?php echo $_SESSION['flag_col']; ?>) {
                ...
                if (!<?php echo checkAllRoomsFound() ? 'true' : 'false'; ?>) {
                    document.querySelector("p").textContent = "Hey wait! You found the way for the next room.";
                    <?php
                    if ($_SESSION['room_number'] < 5) {
                        echo 'setTimeout(function() { window.location.href = "next_room.php"; }, 2000);';
                    }
                    ?>
                } else {
