...
session_start();
session_destroy();
 // Add this line at the beginning of your script to initialize the session.
        ...
        function handleKeyPress(event) {
            ...
            if (currentRow === <?php echo $_SESSION['flag_row']; ?> && currentCol === <?php echo $_SESSION['flag_col']; ?>) {
                ...
                if (!<?php echo checkAllRoomsFound() ? 'true' : 'false'; ?>) {
                    document.querySelector("p").textContent = "Hey wait You found the way for next room";
                    <?php
                    if ($_SESSION['room_number'] < 5) {
                        echo 'setTimeout(function() { window.location.href = "room.php"; }, 2000);';
                    }
                    ?>
                } else {
